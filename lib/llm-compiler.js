...
async function saveCompiledPages(response) {
  ...
  for (const block of blocks) {
    ...
    logger.info(`[llm-compiler] Saved: ${type}/${slugify(filename)}`);
  }
  
  logger.info(`[llm-compiler] Created ${saved.concepts} concepts, ${saved.articles} articles`);
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const result = await compileWithLLM([], { force });
  if (!result.success) {
    process.exit(1);
  }
}
